...
record "pe" [d|
  data PrioEvent p m a = PrioEvent
    { handler :: [(p, a -> m ())]
    }
  |]

prioEnqueue :: (Ord p) => p -> x -> [(p, x)] -> [(p, x)]
prioEnqueue p f xs = as ++ (p, f) : bs
  where
    (as, bs) = span ((< p) . fst) xs

emptyPrioEvent :: PrioEvent p m a
emptyPrioEvent = PrioEvent []

addHandler
  :: (Ord p, MonadState s m, SomeField f)
  => f s (PrioEvent p m a) -> p -> (a -> m ())
  -> m ()
addHandler f p h = modf (f // peHandler) (prioEnqueue p h)

runHandler
  :: (MonadState s m, SomeField f)
  => f s (PrioEvent p m a) -> a
  -> m ()
runHandler f x = getf (f // peHandler) >>= sequence_ . map (($ x) . snd)

record "bind" [d|
  data Bind p m a = Bind
    { value :: a
    , changed :: PrioEvent p m a
    }
  |]

mkBind :: a -> Bind p m a
mkBind x = Bind x emptyPrioEvent
...
putb
  ...
  => f s (Bind p m a) -> a
  -> m ()
putb f x = putf (f // bindValue) x >> runHandler (f // bindChanged) x

modb
  :: (MonadState s m, SomeField f)
  => f s (Bind p m a) -> (a -> a)
  -> m ()
modb f g = getb f >>= putb f . g
